...
A single GameSession represents one game in progress. It is owned by the
QuizifyManager and emits state changes via async callbacks (used by the
WebSocket layer to broadcast to subscribers).
"""
...
class GameSession:
    ...
    def add_player(self, name: str) -> Player:
        ...
        # Avoid name collisions for clarity in the UI.
        existing_names = {p.name.lower() for p in self.players.values()}
        base_name = name.strip()[:20] or "Player"
        ...
        if self.players and self.state != STATE_LOBBY:
            avg = sum(p.score for p in self.players.values()) // len(self.players)
            player.score = avg
    ...
    async def _emit(self, event_type: str, payload: dict[str, Any]) -> None:
        ...
        # Snapshot listeners — they may unsubscribe during iteration.
    ...
    async def _run_round(self) -> None:
        """Drive the full round through to the scoreboard."""
        try:
            while self.current_index + 1 < len(self.questions) and not self._cancelled:
    ...
    async def _next_question(self) -> None:
        ...
        # Wait for either the question window to elapse or for every
        # player to answer — whichever comes first.
        try:
    ...
    async def submit_answer(self, player_id: str, answer_index: int) -> None:
        ...
        points = 0
        if is_correct:
            # Speed bonus: full bonus if answered instantly, none if at deadline.
            speed_ratio = 1.0 - (elapsed / self.settings.question_time)
        ...
        # Early reveal if everyone has answered: signal the round loop.
        if all(self.current_index in p.answers for p in self.players.values()):
