...
class QuizifyManager:
    ...
    async def play_music(self, session: GameSession) -> None:
        ...
        - **Music Assistant + playlist URI**: call ``music_assistant.play_media``
          ...
          MA's radio feature.
        - **Music Assistant + no URI**: skip the MA service (it requires a
          ``media_id`` on current versions) and use the generic
          ``media_player`` path below to shuffle + resume whatever the
          speaker already has queued.
        - **Generic media_player + URI**: ``media_player.play_media``.
        - **Generic media_player + no URI**: shuffle on, then ``media_play``.
        """
        ...
        # Only use music_assistant.play_media when we actually have a
        # media_id (URI). Some Music Assistant versions REQUIRE media_id
        # and reject the call without it ("required key not provided
        # @ data['media_id']"), so for the no-URI case we skip straight
        # to the generic media_player path below, which just resumes /
        # shuffles whatever the speaker already has queued.
        if has_mass and settings.music_uri:
            try:
