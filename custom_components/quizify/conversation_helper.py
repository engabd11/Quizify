...
async def generate_announcement(
    ...
    *,
    ...
    try:
        # blocking=True + return_response=True is the modern HA shape;
        # for older HA versions this returns the response dict directly
        # from the call. We wrap in wait_for so a slow / hung model
        # can't lock up the game.
        ...
        _LOGGER.warning(
            "Conversation agent %s timed out after %.0fs; using fallback",
            agent_id, timeout,
        )
        return None
    except Exception:
        # Service-not-found, validation errors, agent crash — log and
        # fall back. We deliberately swallow all exceptions because a
        # broken AI integration must never break the game.
        _LOGGER.warning(
            "Conversation agent %s failed; using fallback announcement",
