...
class GameSession:
    ...
    def __init__(
        ...
        # Cached intro announcement text, pre-warmed by
        # prepare_start_announcement() before music starts so a slow AI
        # agent doesn't freeze the "Get ready" screen.
        self._start_announcement: str | None = None
    ...
    async def start(self) -> None:
        ...
        Flow:
          1. Move into the ``announcing`` state ("Get ready…").
          2. Resolve the intro text (AI-generated or static template).
          3. Speak it via ``_announce()``, which uses ``blocking=True`` so
             it returns only once playback has actually finished.
          4. Start the question loop.

        Note: we deliberately do NOT poll the media_player state after the
        TTS call. The blocking ``tts.speak`` already gates on playback
        finishing, and the background music plays on the *same* speaker —
        so polling for "not playing" would wait for the music to stop,
        i.e. the whole game, which manifested as a ~35s freeze on the
        announcement screen. The blocking call is the correct, complete
        gate on its own.
        """
        ...
        if self.settings.tts_entity:
            ...
            # _announce() blocks until playback finishes; that's the gate.
            await self._announce(announcement)
        self._task = asyncio.create_task(self._run_round())

    async def prepare_start_announcement(self) -> None:
        """Pre-generate and cache the intro announcement text.

        Called by the websocket layer just before music starts. Doing the
        (bounded) LLM call here means a slow model delays only the brief
        silent pre-roll rather than freezing the "Get ready" screen over
        already-playing music. If no agent is configured this resolves to
        the static template instantly. Safe to call more than once; only
        the first call does the work.
        """
        if self._start_announcement is not None:
            return
        self._start_announcement = await self._generate_start_announcement()

    async def _generate_start_announcement(self) -> str:
        """Resolve the intro text — AI-generated or static template.

        On any AI failure (timeout, model error, empty/garbage response,
        agent missing) we silently fall back to the static template. The
        helper bounds its own runtime, so this never blocks longer than
    ...
    async def _resolve_start_announcement(self) -> str:
        """Return the cached intro text, generating it if not pre-warmed.

        Normally ``prepare_start_announcement()`` has already populated the
        cache before music started, so this is an instant lookup. The
        generation fallback here keeps ``start()`` correct if it's ever
        called without the pre-warm step (e.g. in tests).
        """
        if self._start_announcement is None:
            self._start_announcement = await self._generate_start_announcement()
        return self._start_announcement
    ...
    async def _announce(self, message: str) -> None:
        ...
        if not music_player:
            # tts.speak REQUIRES a media_player_entity_id target. Without a
            # speaker selected there's nowhere to play the announcement, so
            # skip cleanly rather than firing an invalid service call that
            # HA rejects with "required key not provided".
            _LOGGER.debug(
                "TTS configured but no speaker selected; skipping announcement"
            )
            return
        ...
        try:
            service_data: dict[str, Any] = {
                ...
                "message": sanitized,
                "media_player_entity_id": music_player,
                "cache": False,
            }
            # blocking=True: HA waits for the TTS to be played out on the
            # configured media player before returning. This is the sole
            # gate that prevents Q1 appearing while the intro is still
            # being spoken — we no longer poll media_player state, because
            # the background music shares this speaker and polling for
            # "not playing" would wait for the music (i.e. the whole game)
            # to stop.
