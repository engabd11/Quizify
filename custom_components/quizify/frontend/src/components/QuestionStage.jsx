...
export function QuestionStage({
  ...
  // Snapshot remaining time at the moment we pause, so the display freezes.
  ...
  // Capture the frozen remaining time the moment paused flips on, and clear
  // it when paused flips off (the server has already extended the deadline).
  useEffect(() => {
    if (paused && frozenRemainingRef.current === null) {
      frozenRemainingRef.current = Math.max(0, (deadline || 0) - nowSeconds());
    }
    if (!paused) {
      frozenRemainingRef.current = null;
    }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (paused) {
        // While paused, hold whatever remaining time we had at the
        // pause moment.
        if (frozenRemainingRef.current !== null) {
  ...
  }, [deadline, reveal, selected, paused]);
  ...
  const low = remaining < 5 && !reveal && !paused && selected === null;
  ...
  return (
    <div className={`qz-question-stage${paused ? " qz-paused" : ""}`}>
      <div className="qz-progress">
        ...
        <div className="qz-progress-bar">
          <div
            className={`qz-progress-fill${low ? " qz-progress-fill-low" : ""}${paused ? " qz-progress-fill-paused" : ""}`}
            style={{ width: `${percent}%` }}
          />
        </div>
        <div className={`qz-timer ${low ? "qz-timer-low" : ""}${paused ? " qz-timer-paused" : ""}`}>
      ...
      {/* Lifeline buttons — only before answering, not during reveal, not when paused */}
      {!reveal && !paused && onLifeline && (
        <div className="qz-lifelines">
          <button
            type="button"
            className={`qz-lifeline-btn${lifelines?.doublePointsActive ? " qz-lifeline-active" : ""}${lifelines?.doublePointsRequested ? " qz-lifeline-pending" : ""}`}
            onClick={() => onLifeline("doublePoints")}
            disabled={lifelines?.doublePointsActive || lifelines?.doublePointsRequested || selected !== null}
          ...
          >
            ...
            <span className="qz-lifeline-label">
              {lifelines?.doublePointsActive
                ? "2× ON!"
                : lifelines?.doublePointsRequested
                  ? "Arming…"
                  : "2× or −1000"}
            </span>
          </button>
        ...
        {question.answers.map((answer, i) => {
          ...
          return (
            <button
              ...
              onClick={() => !reveal && !paused && selected === null && onAnswer && onAnswer(i)}
              disabled={reveal || paused || selected !== null || !onAnswer}
