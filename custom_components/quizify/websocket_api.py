...
async def ws_game_start(
    ...
    if session is None:
        ...
        return
    # Resolve the intro announcement text BEFORE starting the music. If an
    # AI agent is configured this is where the (bounded) LLM call happens;
    # doing it here means a slow model delays only the silent pre-roll, not
    # a stretch of already-playing music with a frozen "Get ready" screen.
    # The result is cached on the session and consumed by session.start().
    await session.prepare_start_announcement()
