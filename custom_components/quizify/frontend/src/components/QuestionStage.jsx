...
export function QuestionStage({
  ...
  const totalTime = (deadline || 0) - (question?.startedAt || nowSeconds());
  const percent = deadline
    ? Math.max(0, Math.min(100, (remaining / Math.max(1, totalTime)) * 100))
    : 100;
        ...
        {question.answers.map((answer, i) => {
          ...
          return (
            <button
              ...
              disabled={reveal || selected !== null}
            >
              <div className="qz-answer-letter">{LETTERS[i]}</div>
              <div>{answer}</div>
            </button>
          );
        })}
